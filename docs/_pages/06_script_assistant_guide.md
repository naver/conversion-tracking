...
**(STEP 1) 브라우저 창을 새로 엽니다.**

**(STEP 2) `어시스턴트`앱을 엽니다.** <br>
...
**(STEP 3) 전환 테스트를 할 광고를 찾아서 클릭합니다.** <br>
...
**(STEP 4) 사이트에 랜딩되면 로그발생, 쿠키생성, 페이지뷰(PV), 네이버 파라미터 발생 여부를 확인합니다.** 
...
(d) 광고를 클릭했음에도 불구하고, 네이버 광고 쿠키가 0건이거나, 2건 이하인 경우 <br>
 ...
 <br>
(e-1) 광고를 클릭해서 사이트에 들어갔음에도 불구하고, 네이버 파라미터 `미발생` 인 경우 <br>
 . 광고 클릭후 네이버 매체에서 광고 Link_URL에 붙여주는 NaPm파라미터가, 사이트에서 redirection되면서 사라진 경우 <br>
 <br>
(e-2) 광고를 클릭해서 사이트에 들어갔고, 네이버 파라미터(NaPm)가 발생했으나 ci 값, tr 값이 없는 경우 <br>
. 광고 클릭후 네이버 매체에서 광고 Link_URL에 붙여주는 NaPm파라미터가, 사이트에서 redirection되면서 값이 인코딩(encoding) 혹은 디코딩(decoding)되는 경우<br>
<br>
도움말) https://naver.github.io/conversion-tracking/pages/01_script_guide_wcstrans/#q1 의 (a-2) 경우<br>
<br>
이 경우 어시스턴트 화면 제일 하단 '네이버 파라미터' 부분을 보면 다음과 같이 NaPm 값은 있으나, ci, tr 값은 안보이게 됩니다. <br>
![06_14_e2.png]({{"/assets/img/06_14_e2.png"| relative_url}})
<br>
...
**(STEP 5) (전자상거래 사이트라면) 장바구니담기, 구매완료 등의 프로세스를 진행합니다.** <br>
