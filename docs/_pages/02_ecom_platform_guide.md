...
Q3) 카페24 임대몰에서 전환수는 있는데, 전환매출액이 보이지 않습니다. 오류가 있는 것 아닐까요?
A) 카페24 임대몰에서는, `4개 전환`(`구매완료`, `장바구니`, `상품상세보기`, `결제시작`) 이 모두 발생합니다. 이 중 보고서에 보여지는 전환은 `구매완료`, `장바구니` 2가지이며, `구매완료`는 전환 매출액이 있으나, `장바구니`는 전환매출액이 0원입니다. `장바구니` 전환만 발생한 경우라면 전환수는 있는데, 전환매출액은 0원으로 보이게 됩니다. '전환유형별'로 볼 수 있는 보고서 (예: 검색광고의 경우 '다차원보고서' 중 '전환보고서')로 보고서 조회를 하시면 `구매완료`는 없이 `장바구니`전환만 발생한 것을 확인하실 수 있을 것입니다.
