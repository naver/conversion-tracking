---
title: 네이버 광고 전환 추적을 위한 임대몰 설정가이드
layout: post
lesson: 2
---
------
...
※ 참고 <br>
카페24는 `네이버공통인증키` 를 임대몰 Admin UI에 설정/등록하시면, 다음과 같은 전환유형이 자동으로 동작/발생하기 시작합니다. <br>
 . 7/9(화) 까지: `구매완료` 1가지 전환유형 <br>
 . 7/10(수) 약 (새벽) 03시 부터: `구매완료`, `상품상세보기`, `장바구니담기`, `결제시작` 4가지 전환유형 <br>
7/10(수) 광고보고서 부터는 `구매완료` 외에 `장바구니담기`전환도 보고서에 보이게 됩니다. (참고: `상품상세보기`, `결제시작` 전환은 보고서에 표시되지 않는 전환입니다.)<br>
