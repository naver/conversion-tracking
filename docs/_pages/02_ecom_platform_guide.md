...
③ 부가서비스 ▶ (좌측 메뉴) 채널 ▶ 네이버 연동 설정 ▶ `공통 인증키` 부분에 공통인증키 입력 & 중복체크 버튼 클릭
...
![메이크샵_01]({{"/assets/img/02_makeshop_01.png"| relative_url}})

![메이크샵_02]({{"/assets/img/02_makeshop_02.png"| relative_url}})
...
아임웹(imweb)에서는 임대몰 Admin UI에 사이트ID(=네이버공통키, na_account_id)를 입력함으로써 5가지 전환유형(구매완료, 회원가입, 장바구니, 신청예약, 기타)이 설정이 됩니다. 
설정 방법은 다음과 같습니다. 
...
・ 장바구니 담기 : 특정 상품 장바구니 담기 시 추적  <br>
・ 결제 및 구매완료 : 상품 결제 완료 시 추적  <br>
・ 신청 및 예약 완료 : 예약 결제 완료 시 추적 (위젯 설정 필요)  <br>
・ 기타 페이지 추적 : 입력폼 작성 완료 시 추적 (위젯 설정 필요)  <br>
(전환 발생 위치 관련 문의는 아임웹으로 문의부탁드립니다. 아임웹 온라인 가이드: [https://imweb.me/qna](https://imweb.me/qna))  <br>
